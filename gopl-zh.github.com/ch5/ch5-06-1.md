...
for _, dir := range tempDirs() {
	...
	rmdirs = append(rmdirs, func() {
		os.RemoveAll(dir) // NOTE: incorrect! **需要注意，函数值中记录的是循环变量的内存地址，而不是循环变量某一时刻的值**
	})
}
```
...
for i := 0; i < len(dirs); i++ {
	...
	rmdirs = append(rmdirs, func() {
		os.RemoveAll(dirs[i]) // NOTE: incorrect! 错误的
	})
}
```
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNzY2MDgwODgwLC0xODk0MDkzNjgyLDk4Nz
AwOTI4OF19
-->
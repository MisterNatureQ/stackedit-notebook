...
E:\>go test -v -race -bench=. gopl.io/ch8/cake
goos: windows
goarch: amd64
pkg: gopl.io/ch8/cake
Benchmark-4                                    5         238830480 ns/op
BenchmarkBuffers-4                             5         241791660 ns/op
BenchmarkVariable-4                            5         273972680 ns/op
BenchmarkVariableBuffers-4                     5         257336100 ns/op
BenchmarkSlowIcing-4                           1        1040583700 ns/op
BenchmarkSlowIcingManyIcers-4                  5         281328020 ns/op
PASS
ok      gopl.io/ch8/cake        12.289s
```
...
func (s *Shop) inscriber(iced <-chan cake) {
	// ??这里为什么不从channel 中 range  应为这里在main goroutine 中做处理?
   for i := 0; i < s.Cakes; i++ {  
   	   // 从 iced 中读取
      c := <-iced  
      if s.Verbose {  
...
E:\Soft\cmder>go test -v -race -bench=. gopl.io/ch8/cake
goos: windows
goarch: amd64
pkg: gopl.io/ch8/cake
Benchmark-4                                    5         236683060 ns/op
BenchmarkBuffers-4                             5         233334080 ns/op
BenchmarkVariable-4                            5         276938380 ns/op
BenchmarkVariableBuffers-4                     5         257899560 ns/op
BenchmarkSlowIcing-4                           1        1033602400 ns/op
BenchmarkSlowIcingManyIcers-4                  5         276122960 ns/op
PASS
ok      gopl.io/ch8/cake        12.214s
意味着循环执行了 5次，每次循环花费 282 纳秒(ns)
1,000,000,000 纳秒 = 1秒
1000 毫秒 = 1秒
```
...
func Benchmark(b *testing.B) {  
 ...
 // Each step takes exactly 10ms.  No buffers.  cakeshop := defaults  
   cakeshop.Work(b.N) // 224 ms  
}  
  
func BenchmarkBuffers(b *testing.B) {  
   // Adding buffers has no effect.  添加缓存没有效果
  cakeshop := defaults  
   cakeshop.BakeBuf = 10  
  cakeshop.IceBuf = 10  
  cakeshop.Work(b.N) // 224 ms  
...
func BenchmarkVariable(b *testing.B) {  
 ...
 // increases total time due to channel delays.  cakeshop := defaults  
 // 提升每一步的效率
...
func BenchmarkVariableBuffers(b *testing.B) {  
 ...
 // delays resulting from variability.  cakeshop := defaults  
 // 提升每一步的效率 增加缓存
   cakeshop.BakeStdDev = cakeshop.BakeTime / 4  
  cakeshop.IceStdDev = cakeshop.IceTime / 4  
  cakeshop.InscribeStdDev = cakeshop.InscribeTime / 4  
...
func BenchmarkSlowIcing(b *testing.B) {  
 ...
 // adds directly to the critical path.  cakeshop := defaults  
 // 延时中间一个环节的时间
...
func BenchmarkSlowIcingManyIcers(b *testing.B) {  
 ...
 // to its sequential component, following Amdahl's Law.  cakeshop := defaults  
 // 延时 并增加并发量 有明显效果
   cakeshop.IceTime = 50 * time.Millisecond  
  cakeshop.NumIcers = 5  
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjAwMjAxMzA2LC0xODg4NDc0ODQ4LDE4NT
M4MjE4NTksMTE3MDA4MTYyNSwtOTM1ODY3MzQ1LC0xNDMwMjEx
NTI0LC0yNjg0NTU0NTksLTEyNzkwODM2OTcsLTM0NTg2ODA1NS
wtMjY2MDE3NjEzLDE2OTgzMTg3MDcsLTExMTc5MTExNzUsLTUx
OTM5MzcwOCwzMTg0ODc4NDIsNDg3MTQ1NTM2LC0zMjI0MDg2MT
gsLTg1MTAyMTE2MiwtMTI0MjM3NjU1NF19
-->
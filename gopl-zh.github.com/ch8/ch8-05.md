...
func makeThumbnails2(filenames []string) {
	for _, f := range filenames {
		// 虽然 启动了 goroutine 但是 没有等待它们一直到执行完毕 
...
func makeThumbnails3(filenames []string) {
	...
	for _, f := range filenames {
		go func(f string) {
			...
			// 通知 main goroutine 完成
			ch <- struct{}{}
		}(f) // 注意我们将f的值作为一个显式的变量传给了函数，而不是在循环的闭包中声明 这里是 捕获迭代变量 的问题 和匿名函数什么的没有任何关系ch <- struct{}{}
		}(f)
	}
	// Wait for goroutines to complete.
	for range filenames {
		// 阻塞直到所有子 goroutine 执行完成
		<-ch
	}
}
```
...
回忆一下之前在**5.6.1节**中，**中，匿名函数中的循环变量快照问题。上面这个单独的变量f是被所有的匿名函数值所共享，且会被连续的循环迭代所更新的。当新的goroutine开始执行字面函数时，for循环可能已经更新了f并且开始了另一轮的迭代或者（更有可能的）已经结束了整个循环，所以当这些goroutine开始读取f的值时，它们所看到的值已经是slice的最后一个元素了。显式地添加这个参数，我们能够确保使用的f是当go语句执行时的“当前”那个f**。
...
func makeThumbnails4(filenames []string) error {
	// goroutine 之间通信一般是通过 channel
	errors := make(chan error)

	for _, f := range filenames {
		go func(f string) {
			_, err := thumbnail.ImageFile(f)
			// 当它遇到第一个非nil的error时会直接将error返回到调用方，使得没有一个goroutine去排空errors channel。这样剩下的worker goroutine在向这个channel中发送值时，都会永远地阻塞下去，并且永远都不会退出
			errors <- err
		}(f)
	}
	...
	for range filenames {
		// 处理Error
		// 这里还是会尝试对应次数的 读取 但是 return 了 就会导致 goroutine leakerrors <- err
		}(f)
	}

	for range filenames {
		if err := <-errors; err != nil {
			return err // NOTE: incorrect: goroutine leak! 错误 goroutine 泄露 return 导致可能有 goroutine 不能回收
		}
	}
...
这个程序有一个**微妙的bug**。当它遇到第一个非nil的error时会直接将error返回到调用方，使得没有一个goroutine去排空errors channel。这样剩下的worker goroutine在向这个channel中发送值时，都会永远地阻塞下去，并且永远都不会退出。这种情况叫做goroutine泄露（§8.4.4），**可能会导致整个程序卡住或者跑出out of memory的错误**。
...
func makeThumbnails5(filenames []string) (thumbfiles []string, err error) {
	...
	// 创建的 channel 带缓存 这样不会使得 go routine 泄露
...
func makeThumbnails6(filenames <-chan string) int64 {
	...
	for f := range filenames {
		// Add是为计数器加一，必须在worker goroutine开始之前调用，而不是在goroutine中；否则的话我们没办法确定Add是在"closer" goroutine调用Wait之前被调用
		wg.Add(1)
		// worker
		go func(f string) {
			// 注意Add和Done方法的不对称
			// 使用defer来确保计数器即使是在出错的情况下依然能够正确地被减掉wg.Add(1)
		// worker
		go func(f string) {
			defer wg.Done()
	...
	// closer
	// 单独的 goroutine 等待 监控的 goroutine 结束
	go func() {
		// 如果等待操作被放在了main goroutine中，在循环之前，这样的话就永远都不会结束了 因为channel 是无缓存的 会阻塞 work channel
		// 如果等待操作被放在了main goroutine中，如果在循环之后，那么又变成了不可达的部分，因为没有任何东西去关闭这个channel，这个循环就永远都不会终止。
		wg.Wait()
		// 主要是为了 close 的通信作用??
		// 主要是通过 close 结束 channel 的 range
		close(sizes)
	}()

	var total int64
	// 从 channel 中读取数据
	// 在 main goroutine 中读取数据 防止 work goroutine 阻塞go func() {
		wg.Wait()
		close(sizes)
	}()
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzOTM3NDE2MTksODA3NDc2NzcxLC01OD
g1MDM2MjcsMTkyMjQyMTk2MSwtNDA4MTQ2MTgyLC05MjY5ODg1
ODQsMTQxMjMxNDU3OCw1Nzk3MTQ5NjIsLTIwNzk2NTI3MjEsLT
EwNTY2MDczNzksLTUwNTUxNzUwMywtMzE1MDk1NTA0LDEyODkz
OTc4MDhdfQ==
-->
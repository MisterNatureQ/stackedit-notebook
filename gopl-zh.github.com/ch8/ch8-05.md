...
func makeThumbnails3(filenames []string) {
	...
	for _, f := range filenames {
		go func(f string) {
			...
			ch <- struct{}{}
		}(f) // 注意我们将f的值作为一个显式的变量传给了函数，而不是在循环的闭包中声明 这里是 捕获迭代变量 的问题 和匿名函数什么的没有任何关系
	}
...
回忆一下之前在**5.6.1节**中，**匿名函数中的循环变量快照问题。上面这个单独的变量f是被所有的匿名函数值所共享，且会被连续的循环迭代所更新的。当新的goroutine开始执行字面函数时，for循环可能已经更新了f并且开始了另一轮的迭代或者（更有可能的）已经结束了整个循环，所以当这些goroutine开始读取f的值时，它们所看到的值已经是slice的最后一个元素了。显式地添加这个参数，我们能够确保使用的f是当go语句执行时的“当前”那个f**。
...
func makeThumbnails4(filenames []string) error {
	...
	for range filenames {
		// 处理Error
		// 这里还是会尝试对应次数的 读取 但是 return 了 就会导致 goroutine leak
...
func makeThumbnails6(filenames <-chan string) int64 {
	...
	var total int64
	// 从 channel 中读取数据
	// 在 main goroutine 中读取数据 防止 work goroutine 阻塞
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbODA3NDc2NzcxLC01ODg1MDM2MjcsMTkyMj
QyMTk2MSwtNDA4MTQ2MTgyLC05MjY5ODg1ODQsMTQxMjMxNDU3
OCw1Nzk3MTQ5NjIsLTIwNzk2NTI3MjEsLTEwNTY2MDczNzksLT
UwNTUxNzUwMywtMzE1MDk1NTA0LDEyODkzOTc4MDhdfQ==
-->
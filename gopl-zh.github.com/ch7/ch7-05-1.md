...
func main() {
	...
	f(buf) // NOTE: subtly incorrect! 错误
	if debug {
		// ...use buf...
	}
}
...
当main函数调用函数f时，它给f函数的out参数赋了一个\*bytes.Buffer的空指针，所以out的动态值是nil。然而，它的动态类型是\*bytes.Buffer，意思就是out变量是**一个包含空指针值的非空接口（如图7.5），所以防御性检查out!=nil的结果依然是true**。
...
```go
var buf io.Writer // 可以避免一开始就将一个不完整的值赋值给这个接口 buf 默认就是 nil 
if debug {
	buf = new(bytes.Buffer) // enable collection of output
}
f(buf) // OK
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwODQ4MTIxMTQsLTIwNjE5NTUxNThdfQ
==
-->
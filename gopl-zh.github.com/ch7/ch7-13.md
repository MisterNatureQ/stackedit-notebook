...
接口被以两种不同的方式使用。在**第一个方式中，以io.Reader，io.Writer，fmt.Stringer，sort.Interface，http.Handler和error为典型，一个接口的方法表达了实现这个接口的具体类型间的相似性，但是隐藏了代码的细节和这些具体类型本身的操作。重点在于方法上，而不是具体的类型上**。
...
```go
go doc driver.NamedValue
go doc driver.Value
go doc sql.Exec
go doc sql.NamedArg
// 这个方法处理的参数列表
go doc sql.convert.driverArgsConnLocked 
// 这个方法转换成完整的语句
go doc sql.ctxutil.ctxDriverExec
新的方法还没有理解  中间 np,  ok  :=  arg.(NamedArg) 这里发生了什么

https://github.com/golang/go/blob/master/src/database/sql/convert.go#L105:6
https://github.com/golang/go/blob/master/src/database/sql/sql.go#L81:6
```

```go
// 现在版本的好像没有同样的代码了 用更新的方式替换了
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzMzMzODU4NzMsMzkzMjE0ODMxLDE4MT
k4NTc4MTksLTE2NTQ0MjY0NjksMTQ1MDUxNTYyNSw5NDI0NzM3
MDUsLTE4NTQ4MDA0ODgsNjg5MjE4NzI3LDEzOTEwOTI4NTIsLT
EyNTAwMjE1MjgsLTE5MTMzMTA0LDkxMzMwMzgwOCwxNTg5NjIw
NDE2LDE3MDAyMzEyMTQsLTcwODQyMzgxMywtNjU1NDM2MDM3XX
0=
-->
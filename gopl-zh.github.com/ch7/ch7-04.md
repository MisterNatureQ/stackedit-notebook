...
让我们定义一个允许通过摄氏度或者华氏温度变换的形式指定温度的celsiusFlag类型。注意celsiusFlag内嵌了一个Celsius类型（§2.5 自己实现了 并不是继承什么的），因此**不用实现本身就已经有String方法了**。为了实现flag.Value，我们只需要定义Set方法：
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTkwMTg1OTQ1MywtMTgyNDA5NjU4LC0xMj

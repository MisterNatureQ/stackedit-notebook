...
// 因为是指针类型*errorString满足error接口而非errorString类型，所以每个New函数的调用都分配了一个独特的和其他错误不相同的实例 ?? 没有体会出不同 如果不是指针类型就不行吗？ 比较的是 string 就有可能相等
...
```go
fmt.Println(errors.New("EOF") == errors.New("EOF")) // "false" 如果不是指针类型就不行吗？ 比较的是 string 就有可能相等
```
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTcwNjkzNDkyOSwtMTkyMjU0MDYyNSwtMT
U5MDMwODEwNiwyNzMwNzA0NzYsLTIyODE5MDk2NV19
-->
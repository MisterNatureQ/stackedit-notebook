...
**在一个程序中有非并发安全的类型的情况下，我们依然可以使这个程序并发安全。确实，并发安全的类型是例外，而不是规则，所以只有当文档中明确地说明了其是并发安全的情况下，你才可以并发地去访问它**。我们会避免并发访问大多数的类型，无论是**将变量局限在单一的一个goroutine内**，还是**用互斥条件维持更高级别的不变性**，都是为了这个目的。我们会在本章中说明这些术语。
...
// 出纳员
func teller() {
	// 被限制在 goroutine 中的变量 **不要使用共享数据来通信；使用通信来共享数据**
...
```go
// **串行绑定** 类似于 将共享变量的放在goroutine 中 来隔绝并发访问 来解决数据竞争的问题
// ?? 还没有衔接起来这个流水线
...
func baker(cooked chan<- *Cake) {
	for {
		cake := new(Cake)
		cake.state = "cooked"
		// 其效果是变量会被绑定到流水线的一个阶段，传送完之后被绑定到下一个，以此类推。这种规则有时被称为**串行绑定**
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTgyMTM5Njk0OCwtNTI4MzY1NDU0LC04MD
IwNDkzNCwtMjA0NjkzNDI3OCwxMjc1OTM3MzU0LDE4NTI4Mjc1
LC0yMDIyNzI0MjY5LDU1NzQzOTU1LDczMDA5NjgxNSwtOTYzND
MyNzIxLDgwNzU1NjgwNCwtODcxOTMzMTQ5LDEzNzMwNjk0NjMs
LTIwNjk3MzQ2MTksLTkxMTk1NDU4NywxNzA2NzY5MzgxLDE0NT
QzNTE5NzMsLTE1ODgyNTAwNzVdfQ==
-->
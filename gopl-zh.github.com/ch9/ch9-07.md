...
32          memo.cache[key] = res // -race 检测 出现了两次
33      }
...
测试依然并发进行，但这回竞争检查器“沉默”了。不幸的是对于Memo的**这一点改变使我们完全丧失了并发的性能优点**。每次对f的调用期间都会持有锁，Get将本来可以并行运行的I/O操作串行化了。我们本章的目的是完成一个无锁缓存，而不是现在这样的将所有请求串行化的函数的缓存。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwMjc5NDkyNDgsMTQ2NzE5NTgzNCwxOD
IyMzQ5OTQ2LC0xMDIzNzQ5MzQ1LDQ1NTUwNDIxNiwxMDk2NTM0
MDU1LDEyNjE4NTU5NTksODI1Njg3MTE0LDE4NjY3OTUyMjksMT
U4MzI2NTM1OCwxNTg3MzY5NDI3LDEzNzIyMTA5NTUsLTExODY5
MjMxNzgsLTE2MzUzODQ5Ml19
-->
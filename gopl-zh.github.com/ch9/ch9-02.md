...
当然，这里的存款deposit函数很小，实际上取款Withdraw函数不需要理会对它的调用，尽管如此，这里的表达还是表明了规则。
	...
	封装一个对象的变量或者方法如果对调用方是不可见的话，一般就被定义为“封装”。封装有时候也被叫做信息隐藏，同时也是面向对象编程最关键的一个方面
...
**封装（§6.6），用限制一个程序中的意外交互的方式，可以使我们获得数据结构的不变性。因为某种原因，封装还帮我们获得了并发的不变性。当你使用mutex时，确保mutex和其保护的变量没有被导出（在go里也就是小写，且不要被大写字母开头的函数访问啦），无论这些变量是包级的变量还是一个struct的字段。**
	...
	就是是建议 mutex 和管理关联的数据通过 struct 绑定起来 并且通过小写命名 使其外部不可访问
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTI5ODE2MTk5NywtMTc3MzYwNjg3LDE0Mj
c5MTI2NzcsMjAwMTE4MzQwLDIzNTgzMTY2NiwtMTkxMTc0MzI2
MywtMjA5NTc3ODgyNiwtNjMyMjA0ODQxLDY1ODYwOTczLDE0Nz
g1NzI5MzcsMTkyNjE4NDc5Myw3NTE3MDk3NTVdfQ==
-->
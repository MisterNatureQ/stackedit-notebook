...
这个被称为**包的匿名导入。它通常是用来实现一个编译时机制，然后通过在main主程序入口选择性地导入附加的包**。首先，让我们看看如何使用该特性，然后再看看它是如何工作的。
...
import (
	...
	"image"
	"image/jpeg"
	_ "image/png" // register PNG decoder 包的匿名导入。它通常是用来实现一个编译时机制，然后通过在main主程序入口选择性地导入附加的包
	"io"
	"os"
)
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMzI1OTU4MzkxLDgzMDY2Njk5OSwyMDkwND
cxODNdfQ==
-->